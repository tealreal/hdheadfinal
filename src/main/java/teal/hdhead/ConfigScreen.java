...
public final class ConfigScreen implements ModMenuApi {
    private final ConfigObject virginConfig = new ConfigObject();
    @Override
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return parent -> {
            ...
            ux.addEntry(eb.startBooleanToggle(Text.literal("Run Mod"), HDHeads.doRunMod())
                .setDefaultValue(true)
                .setTooltip(Text.literal("Whether to run the mod"))
                .setSaveConsumer(HDHeads::setRunMod)
                .build()
            );
            ...
            ux.addEntry(eb.startLongField(Text.literal("File Size Limit (KB)"), config.getThresholdSize())
                .setDefaultValue(virginConfig.getThresholdSize())
                .setTooltip(Text.literal("The limit on how big a head's file size can be before being blocked. (-1 to disable)"))
                .setMin(-1)
                .setSaveConsumer(k -> config.setThresholdSize(1000 * k))
                .build()
            );
            ...
            fun.addEntry(eb.startIntList(Text.literal("Image "), List.of(config.getScaleInject()))
                ...
                .setSaveConsumer(s -> config.setScaleInject(s.toArray(new Integer[0])))
                .build()
            );
